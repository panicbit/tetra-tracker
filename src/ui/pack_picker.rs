...
use egui::{CentralPanel, ScrollArea, SidePanel, Ui, Widget};
...
impl PackPicker {
    ...
    pub fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) -> Option<Pack> {
        ...
        SidePanel::right("pack_picker_right_side").show(ctx, |ui| {
            ScrollArea::vertical().show(ui, |ui| {
                let Some((manifest_path, manifest)) = pack_manifests.get(self.selected_manifest)
                else {
                    return;
                };
